...
io.on("connection", (socket) => {
  ...
  socket.on("audio-chunk", (chunk) => {
    console.log("⬅️ AUDIO-CHUNK RECEIVED from:", socket.id);
    console.log("    TYPE:", typeof chunk);
    console.log("    KEYS:", chunk ? Object.keys(chunk) : "null");
    console.log("    RAW:", chunk);

    // Python sends: { type:'Buffer', data:[...] }
    if (chunk && chunk.data) {
      console.log("    🔧 Python chunk detected. Converting...");
      chunk = Buffer.from(chunk.data);
    }

    // Browser sends ArrayBuffer
    if (chunk instanceof ArrayBuffer) {
      console.log("    🔧 Browser ArrayBuffer detected. Converting...");
      chunk = Buffer.from(chunk);
    }

    const roomsJoined = Array.from(socket.rooms).filter((r) => r !== socket.id);
    roomsJoined.forEach((room) => {
      console.log("    📤 Relaying audio to room:", room);
